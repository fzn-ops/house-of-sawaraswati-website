            ...
            <tr class="border-b border-gray-50 dark:border-gray-800 hover:bg-rose-50/30 dark:hover:bg-rose-900/10 transition-colors">
                ...
                <td class="px-4 py-3">
                    <div class="flex items-center justify-center gap-2">
                        ${row.status === 'pending' && row.snap_token ? `
                        <button onclick="openPayment('${row.snap_token}', '${row.order_id}')" title="Bayar"
                                class="w-7 h-7 flex items-center justify-center text-purple-400 hover:text-purple-600 transition-colors">
...
// ===== BAYAR (OPEN MIDTRANS SNAP) =====
function openPayment(snapToken, orderId) {
    if (!snapToken) {
...
function updatePaymentStatus(orderId, status) {
    ...
    .then(data => {
        if (data.success) {
            ...
            const filtered = filteredData.find(r => r.order_id === orderId);
            if (filtered) filtered.status = status;
            renderTable();
            updateReportingStats();
        }
    })
